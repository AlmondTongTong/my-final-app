...
const App = () => {
  ...
  useEffect(() => { if (!db || !isAdmin) return; setGradedPosts(new Set()); const questionsQuery = query(collection(db, `/artifacts/${appId}/public/data/questions`), where("course", "==", selectedCourse), where("date", "==", adminSelectedDate)); const unsubQ = onSnapshot(questionsQuery, (snap) => setQuestionsLog(snap.docs.map(d => ({ id: d.id, ...d.data() })))); const feedbackQuery = query(collection(db, `/artifacts/${appId}/public/data/feedback`), where("course", "==", selectedCourse), where("date", "==", adminSelectedDate)); const unsubF = onSnapshot(feedbackQuery, (snap) => setFeedbackLog(snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => b.timestamp - a.timestamp))); return () => { unsubQ(); unsubF(); }; }, [db, selectedCourse, adminSelectedDate, appId, isAdmin]);
    ...
    <div className="w-full max-w-lg p-6 bg-slate-800 text-white rounded-xl shadow-lg box-shadow-custom">
      {isAdmin ? (
        <>
          ...
          <div className="text-left p-4 border border-slate-600 rounded-xl mt-6"> <h3 className="text-xl font-semibold text-gray-100 mb-4">🏆 {selectedCourse} Talent Leaderboard</h3> <TalentGraph talentsData={talentsLog} type="admin" /> </div>
        </>
      ) : (
        <>
          ...
          {isNameEntered && !isAuthenticated && <PinAuth nameInput={nameInput} isPinRegistered={isPinRegistered} onLogin={handlePinLogin} onRegister={handlePinRegister} />}
          ...
          {isAuthenticated && (
            <div className="mt-4 animate-fade-in">
              ...
              <div className="text-left p-4 border border-slate-600 rounded-xl mt-6"> <h3 className="text-xl font-semibold text-gray-100 mb-4">Class Score Range</h3> <TalentGraph talentsData={talentsLog} type="student" /> </div>
            </div>
          )}
        </>
      )}
