...
const App = () => {
  ...
  const handleAddContent = useCallback(async (text, type) => { if (!db || !nameInput.trim() || !text.trim()) return; const today = new Date().toISOString().slice(0, 10); try { const docRef = await addDoc(collection(db, `/artifacts/${appId}/public/data/questions`), { name: nameInput, text, type, course: selectedCourse, date: today, timestamp: serverTimestamp(), liked: false }); showMessage("Submission complete! ✅"); await modifyTalent(nameInput, 1, 'automatic', docRef.id); } catch (e) { showMessage("Submission failed. ❌"); } }, [db, nameInput, selectedCourse, appId, modifyTalent, showMessage]);
