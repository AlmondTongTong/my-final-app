...
const TalentGraph = ({ talentsData, type, selectedCourse, getFirstName }) => {
    ...
    return ( <div className="space-y-4"> {displayData.map(talent => ( <div key={talent.id} className="w-full"> <div className="flex justify-between text-lg text-gray-300 mb-1"> <span>{type === 'admin' ? getFirstName(talent.name) : (talent.id === displayData[0].id ? 'Highest Score' : 'Lowest Score')}</span> <span>{talent.totalTalents}</span> </div> <div className="w-full bg-slate-600 rounded-full h-5"> <div className="bg-yellow-400 h-5 rounded-full" style={{ width: maxScore > 0 ? `${(talent.totalTalents / maxScore) * 100}%` : '0%' }} ></div> </div> </div> ))} </div> );
};

const ContentForm = React.memo(({ type, onAddContent, isEnabled, placeholder }) => { const [text, setText] = useState(''); const handleSubmit = (event) => { event.preventDefault(); if (text.trim()) { onAddContent(text, type); setText(''); } }; return ( <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2"> <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder={placeholder} disabled={!isEnabled} className="flex-1 p-3 border bg-slate-700 border-slate-500 rounded-lg text-2xl resize-none h-28" /> <button type="submit" disabled={!isEnabled || !text.trim()} className="p-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg disabled:opacity-50 self-end sm:self-center text-xl" > Add </button> </form> ); });
const AdminLoginForm = ({ onAdminLogin }) => { const [password, setPassword] = useState(''); const handleLogin = () => { onAdminLogin(password); }; return ( <div className="flex space-x-2"> <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="p-2 border bg-slate-700 border-slate-500 rounded-lg text-lg" /> <button onClick={handleLogin} className="p-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-lg">Login</button> </div> ); };
const PinAuth = React.memo(({ nameInput, isPinRegistered, onLogin, onRegister, getFirstName }) => { const [pinInput, setPinInput] = useState(''); const [pinConfirmationInput, setPinConfirmationInput] = useState(''); const handleLoginClick = () => { onLogin(pinInput); setPinInput(''); }; const handleRegisterClick = () => { onRegister(pinInput, pinConfirmationInput); setPinInput(''); setPinConfirmationInput(''); }; if (!nameInput) return null; return isPinRegistered ? ( <div className="my-4 p-4 bg-slate-700 rounded-lg animate-fade-in"> <p className="text-center text-white mb-2 font-semibold text-2xl">Enter your 4-digit PIN, {getFirstName(nameInput)}.</p> <div className="flex space-x-2"> <input type="password" inputMode="numeric" maxLength="4" value={pinInput} onChange={(e) => setPinInput(e.target.value)} className="flex-1 p-3 border bg-slate-600 border-slate-500 rounded-lg text-2xl text-center"/> <button onClick={handleLoginClick} className="p-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold text-xl">Login</button> </div> </div> ) : ( <div className="my-4 p-4 bg-slate-700 rounded-lg animate-fade-in"> <p className="text-center text-white mb-2 font-semibold text-2xl">First time? Create your 4-digit PIN.<br/><span className="text-lg font-normal">(Use the last 4 digits of your Student ID)</span></p> <div className="space-y-2"> <input type="password" inputMode="numeric" maxLength="4" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="Create 4-digit PIN" className="w-full p-3 border bg-slate-600 border-slate-500 rounded-lg text-2xl text-center"/> <input type="password" inputMode="numeric" maxLength="4" value={pinConfirmationInput} onChange={(e) => setPinConfirmationInput(e.target.value)} placeholder="Confirm PIN" className="w-full p-3 border bg-slate-600 border-slate-500 rounded-lg text-2xl text-center"/> <button onClick={handleRegisterClick} className="w-full p-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-bold text-xl">Register & Start</button> </div> </div> ); });
...
const App = () => {
  ...
  const handleAddReply = useCallback(async (postId, replyText) => { if (!db || !nameInput) return; const repliesColRef = collection(db, `/artifacts/${appId}/public/data/questions/${postId}/replies`); try { await addDoc(repliesColRef, { text: replyText, author: getFirstName(nameInput), authorFullName: nameInput, adminLiked: false, timestamp: serverTimestamp() }); } catch (error) { console.error("Error adding reply: ", error); } }, [db, nameInput, getFirstName, appId]);
  const handleAdminLikeReply = useCallback(async (postId, replyId, authorFullName) => { if(!db) return; const replyDocRef = doc(db, `/artifacts/${appId}/public/data/questions/${postId}/replies`, replyId); try { await updateDoc(replyDocRef, { adminLiked: true }); await modifyTalent(authorFullName, 0.5, 'reply_bonus'); } catch (e) { console.error("Error (admin like reply):", e) } }, [db, appId, modifyTalent]);
  ...
  const [postsWithReplies, postsWithoutReplies] = useMemo(() => { const withReplies = []; const withoutReplies = []; allPostsLog.forEach(post => { if (replies[post.id] && replies[post.id].length > 0) { withReplies.push(post); } else { withoutReplies.push(post); } }); return [withReplies, withoutReplies]; }, [allPostsLog, replies]);
  const [adminPostsWithReplies, adminPostsWithoutReplies] = useMemo(() => { const withReplies = []; const withoutReplies = []; questionsLog.forEach(post => { if (replies[post.id] && replies[post.id].length > 0) { withReplies.push(post); } else { withoutReplies.push(post); } }); return [withReplies, withoutReplies]; }, [questionsLog, replies]);

  const ReplyForm = ({ log, onReply }) => { const [replyText, setReplyText] = useState(''); const handleSend = () => { onReply(log.id, replyText); setReplyText(''); }; return ( <div className="mt-2 flex items-center space-x-2"> <input type="text" value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder={`Reply to ${getFirstName(log.name)}...`} className="flex-1 p-2 border bg-slate-600 border-slate-500 rounded-lg text-lg" /> <button onClick={handleSend} className="p-2 bg-blue-500 hover:bg-blue-600 text-white text-lg rounded-lg">Send</button> <button onClick={() => onReply(log.id, "Addressed in class")} className="p-2 bg-gray-500 hover:bg-gray-600 text-white text-lg rounded-lg whitespace-nowrap">Addressed in Class</button> </div> ); };
  const StudentReplyForm = ({ postId, onAddReply }) => { const [replyText, setReplyText] = useState(''); const handleSend = () => { onAddReply(postId, replyText); setReplyText(''); }; return ( <div className="mt-2 flex items-center space-x-2"> <input type="text" value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Write an anonymous reply..." className="flex-1 p-2 border bg-slate-600 border-slate-500 rounded-lg text-lg" /> <button onClick={handleSend} className="p-2 bg-blue-500 hover:bg-blue-600 text-white text-lg rounded-lg">Reply</button> </div> ); };
  ...
  const MainContent = () => (
    <div className="w-full max-w-4xl p-6 bg-slate-800 text-white rounded-xl shadow-lg box-shadow-custom">
        ...
        <>
          ...
          {adminSelectedStudent ? (
            <div className="text-left p-4 border border-slate-600 rounded-xl mt-6">
              ...
              <ul>{adminStudentLog.map((log) => ( <li key={log.id} className="p-2 border-b border-slate-700 text-xl"> <div className="flex justify-between items-start"> <span className="flex-1 mr-2"><span className="font-bold">{log.date}</span> [{log.type}]: {log.text}</span> <div className="flex items-center space-x-2 flex-shrink-0"> {log.adminLiked ? <span className="text-green-500 font-bold text-lg">✓ Liked</span> : <button onClick={() => handleAdminLike(log.id)} className="text-3xl">👍</button>} <button onClick={() => modifyTalent(log.name, -1, 'penalty')} className="px-3 py-1 bg-red-600 text-white text-md font-bold rounded hover:bg-red-700">-1</button> <button onClick={() => modifyTalent(log.name, 1, 'bonus')} className="px-3 py-1 bg-yellow-500 text-black text-md font-bold rounded hover:bg-yellow-600">+1</button> </div> </div> {log.reply && <div className="mt-2 p-2 bg-slate-700 rounded-lg text-lg text-gray-300"><b>Reply:</b> {log.reply} {log.studentLiked && <span className="ml-2">👍 by student</span>}</div>} <ReplyForm log={log} onReply={handleReply} /> </li> ))}</ul>
            </div>
          ) : (
            <>
              ...
              <div className="text-left p-4 border border-slate-600 rounded-xl mt-6"> 
                <h3 className="text-3xl font-semibold">❓ Daily Posts</h3> 
                <h4 className="font-semibold mt-4 text-2xl text-gray-300">💬 Posts with Student Replies</h4>
                <ul className="h-48 overflow-y-auto text-xl mb-4">{adminPostsWithReplies.map((log) => ( <li key={log.id} className="p-2 border-b border-slate-700"> <div className="flex justify-between items-start"> <span className="flex-1 mr-2">{getFirstName(log.name)} [{log.type}]: {log.text}</span> <div className="flex items-center space-x-2 flex-shrink-0"> {log.adminLiked ? <span className="text-green-500 font-bold text-lg">✓ Liked</span> : <button onClick={() => handleAdminLike(log.id)} className="text-3xl">👍</button>} <button onClick={() => modifyTalent(log.name, -1, 'penalty')} className="px-3 py-1 bg-red-600 text-white text-md font-bold rounded hover:bg-red-700">-1</button> <button onClick={() => modifyTalent(log.name, 1, 'bonus')} className="px-3 py-1 bg-yellow-500 text-black text-md font-bold rounded hover:bg-yellow-600">+1</button> </div> </div> {log.reply && <div className="mt-2 p-2 bg-slate-700 rounded-lg text-lg text-gray-300"><b>Reply:</b> {log.reply} {log.studentLiked && <span className="ml-2">👍 by student</span>}</div>} <ReplyForm log={log} onReply={handleReply} /> <button onClick={() => toggleReplies(log.id)} className="text-lg text-blue-400 mt-1">{showReplies[log.id] ? 'Hide Replies' : 'Show Replies'}</button> {showReplies[log.id] && <div className="mt-2 pl-4 border-l-2 border-slate-500"> <ul className="text-lg mt-2">{replies[log.id]?.map(reply => <li key={reply.id} className="pt-1 flex justify-between items-center"><span>{reply.author}: {reply.text}</span> {reply.adminLiked ? <span className="text-green-500 font-bold text-lg">✓ Liked</span> : <button onClick={() => handleAdminLikeReply(log.id, reply.id, reply.authorFullName)} className="text-3xl">👍</button>}</li>)}</ul></div>} </li> ))}</ul>
                <h4 className="font-semibold mt-4 text-2xl text-gray-300">Posts without Replies</h4>
                <ul className="h-48 overflow-y-auto text-xl">{adminPostsWithoutReplies.map((log) => ( <li key={log.id} className="p-2 border-b border-slate-700"> <div className="flex justify-between items-start"> <span className="flex-1 mr-2">{getFirstName(log.name)} [{log.type}]: {log.text}</span> <div className="flex items-center space-x-2 flex-shrink-0"> {log.adminLiked ? <span className="text-green-500 font-bold text-lg">✓ Liked</span> : <button onClick={() => handleAdminLike(log.id)} className="text-3xl">👍</button>} <button onClick={() => modifyTalent(log.name, -1, 'penalty')} className="px-3 py-1 bg-red-600 text-white text-md font-bold rounded hover:bg-red-700">-1</button> <button onClick={() => modifyTalent(log.name, 1, 'bonus')} className="px-3 py-1 bg-yellow-500 text-black text-md font-bold rounded hover:bg-yellow-600">+1</button> </div> </div> {log.reply && <div className="mt-2 p-2 bg-slate-700 rounded-lg text-lg text-gray-300"><b>Reply:</b> {log.reply} {log.studentLiked && <span className="ml-2">👍 by student</span>}</div>} <ReplyForm log={log} onReply={handleReply} /> <button onClick={() => toggleReplies(log.id)} className="text-lg text-blue-400 mt-1">{showReplies[log.id] ? 'Hide Replies' : 'Show Replies'}</button> {showReplies[log.id] && <div className="mt-2 pl-4 border-l-2 border-slate-500"> <ul className="text-lg mt-2">{replies[log.id]?.map(reply => <li key={reply.id} className="pt-1 flex justify-between items-center"><span>{reply.author}: {reply.text}</span> {reply.adminLiked ? <span className="text-green-500 font-bold text-lg">✓ Liked</span> : <button onClick={() => handleAdminLikeReply(log.id, reply.id, reply.authorFullName)} className="text-3xl">👍</button>}</li>)}</ul></div>} </li> ))}</ul>
              </div>
            </>
          )}
          <div className="text-left p-4 border border-slate-600 rounded-xl mt-6"> <h3 className="text-3xl font-semibold text-gray-100 mb-4">🏆 {selectedCourse} Talent Leaderboard</h3> <TalentGraph talentsData={talentsLog} type="admin" selectedCourse={selectedCourse} getFirstName={getFirstName} /> </div>
        </>
      ) : (
        <>
            ...
            <div className="mt-4 animate-fade-in">
              <div className="text-left p-4 border border-slate-600 rounded-xl mb-6">
                ...
                <ul className="list-disc list-inside text-xl mt-2">
                  ...
                  <li><span className="font-semibold text-yellow-400">Bonus:</span> Get a 'Like' from Prof. Ahn on your reply to a peer: <span className="font-semibold">+0.5 Talents</span></li>
                </ul>
              </div>
              ...
              <div className="flex justify-center items-center text-center my-4 p-3 bg-yellow-400 text-black rounded-lg"> <img src="/talent-coin.png" alt="Talent coin" className="w-8 h-8 mr-2" /> <p className="font-bold text-2xl">My Total Talents: {myTotalTalents}</p> </div>
      ...
      <div className="flex flex-col items-center mt-8 p-4 border-t border-slate-600"> <p className="text-xl font-medium text-gray-200 mb-2">Admin Login</p> <AdminLoginForm onAdminLogin={handleAdminLogin} /> </div>
    </div>
  );
  ...
  const CreatePollForm = ({ onCreatePoll, onDeactivatePoll, activePoll }) => {
    ...
    if (activePoll) { return ( <div className="p-4 border border-slate-600 rounded-xl mb-6"> <h3 className="text-3xl font-semibold">Active Poll Results</h3> <PollComponent poll={activePoll} isAdminView={true} /> <button onClick={() => onDeactivatePoll(activePoll.id)} className="w-full p-2 mt-4 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xl">Close Poll</button> </div> ); }
