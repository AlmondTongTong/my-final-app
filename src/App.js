...
const TalentGraph = ({ talentsData, type, selectedCourse, getFirstName }) => {
    ...
    return ( <div className="space-y-2"> {displayData.map(talent => ( <div key={talent.id} className="w-full"> <div className="flex justify-between text-sm text-gray-500 mb-1"> <span>{type === 'admin' ? getFirstName(talent.name) : (talent.id === displayData[0].id ? 'Highest Score' : 'Lowest Score')}</span> <span>{talent.totalTalents}</span> </div> <div className="w-full bg-gray-200 rounded-full h-4"> <div className="bg-yellow-400 h-4 rounded-full" style={{ width: maxScore > 0 ? `${(talent.totalTalents / maxScore) * 100}%` : '0%' }} ></div> </div> </div> ))} </div> );
};

const ContentForm = React.memo(({ type, onAddContent, isEnabled, placeholder }) => { const [text, setText] = useState(''); const handleSubmit = (event) => { event.preventDefault(); if (text.trim()) { onAddContent(text, type); setText(''); } }; return ( <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2"> <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder={placeholder} disabled={!isEnabled} className="flex-1 p-3 border bg-gray-100 border-gray-300 rounded-lg text-lg resize-none h-24 focus:ring-2 focus:ring-orange-500" /> <button type="submit" disabled={!isEnabled || !text.trim()} className="p-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg disabled:opacity-50 self-end sm:self-center" > Add </button> </form> ); });
const AdminLoginForm = ({ onAdminLogin }) => { const [password, setPassword] = useState(''); const handleLogin = () => { onAdminLogin(password); }; return ( <div className="flex space-x-2"> <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="p-2 border bg-gray-200 border-gray-300 rounded-lg text-sm" /> <button onClick={handleLogin} className="p-2 bg-gray-700 hover:bg-black text-white rounded-lg text-sm">Login</button> </div> ); };
const PinAuth = React.memo(({ nameInput, isPinRegistered, onLogin, onRegister, getFirstName }) => { const [pinInput, setPinInput] = useState(''); const [pinConfirmationInput, setPinConfirmationInput] = useState(''); const handleLoginClick = () => { onLogin(pinInput); setPinInput(''); }; const handleRegisterClick = () => { onRegister(pinInput, pinConfirmationInput); setPinInput(''); setPinConfirmationInput(''); }; if (!nameInput) return null; return isPinRegistered ? ( <div className="my-4 p-4 bg-gray-100 rounded-lg animate-fade-in"> <p className="text-center text-gray-800 mb-2 font-semibold">Enter your 4-digit PIN, {getFirstName(nameInput)}.</p> <div className="flex space-x-2"> <input type="password" inputMode="numeric" maxLength="4" value={pinInput} onChange={(e) => setPinInput(e.target.value)} className="flex-1 p-3 border bg-white border-gray-300 rounded-lg text-lg text-center"/> <button onClick={handleLoginClick} className="p-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold">Login</button> </div> </div> ) : ( <div className="my-4 p-4 bg-gray-100 rounded-lg animate-fade-in"> <p className="text-center text-gray-800 mb-2 font-semibold">First time? Create your 4-digit PIN.<br/><span className="text-sm font-normal">(Use the last 4 digits of your Student ID)</span></p> <div className="space-y-2"> <input type="password" inputMode="numeric" maxLength="4" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="Create 4-digit PIN" className="w-full p-3 border bg-white border-gray-300 rounded-lg text-lg text-center"/> <input type="password" inputMode="numeric" maxLength="4" value={pinConfirmationInput} onChange={(e) => setPinConfirmationInput(e.target.value)} placeholder="Confirm PIN" className="w-full p-3 border bg-white border-gray-300 rounded-lg text-lg text-center"/> <button onClick={handleRegisterClick} className="w-full p-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-bold">Register & Start</button> </div> </div> ); });
...
const App = () => {
  ...
  useEffect(() => {
    ...
    }, (error) => console.error("My Posts Query Error: ", error));
    ...
    const unsubAll = onSnapshot(allPostsQuery, (snap) => setAllPostsLog(snap.docs.map(d => ({id: d.id, ...d.data()}))), (error) => console.error("All Posts Query Error: ", error));
    ...
    const unsubF = onSnapshot(feedbackQuery, (snap) => setStudentFeedbackLog(snap.docs.map(d => d.data())), (error) => console.error("My Feedback Query Error: ", error));
    ...
    <div className="w-full max-w-2xl p-8 bg-white text-gray-800 rounded-2xl shadow-2xl">
      {isAdmin ? (
        <>
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-6"> {COURSES.map((course) => <button key={course} onClick={() => setSelectedCourse(course)} className={`px-4 py-2 text-sm font-medium rounded-full ${selectedCourse === course ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>{course}</button>)} </div>
          <select value={adminSelectedStudent} onChange={(e) => setAdminSelectedStudent(e.target.value)} className="p-3 mb-6 w-full border bg-gray-50 border-gray-300 rounded-lg text-lg"> <option value="">-- View All Daily Logs --</option> {COURSE_STUDENTS[selectedCourse].map((name, i) => <option key={i} value={name}>{name}</option>)} </select>
          {adminSelectedStudent ? (
            <div className="text-left p-4 border border-gray-200 rounded-xl mt-6 bg-gray-50">
              <h3 className="text-xl font-semibold text-gray-900">All Logs for {getFirstName(adminSelectedStudent)}</h3>
              ...
              <ul>{adminStudentLog.map((log) => ( <li key={log.id} className="p-2 border-b border-gray-200"> <div className="flex justify-between items-start"> <span className="flex-1 mr-2"><span className="font-bold text-gray-500 text-sm">{log.date}</span> <span className="font-semibold">[{log.type}]:</span> {log.text}</span> <div className="flex items-center space-x-2 flex-shrink-0"> {log.adminLiked ? <span className="text-green-500 font-bold">✓ Liked</span> : <button onClick={() => handleAdminLike(log.id)} className="text-2xl">👍</button>} <button onClick={() => modifyTalent(log.name, -1, 'penalty')} className="px-2 py-1 bg-red-600 text-white text-xs font-bold rounded hover:bg-red-700">-1</button> <button onClick={() => modifyTalent(log.name, 1, 'bonus')} className="px-2 py-1 bg-yellow-500 text-black text-xs font-bold rounded hover:bg-yellow-600">+1</button> </div> </div> {log.reply && <div className="mt-2 p-2 bg-gray-100 rounded-lg text-sm text-gray-700"><b>Reply:</b> {log.reply} {log.studentLiked && <span className="ml-2">👍 by student</span>}</div>} <ReplyForm log={log} onReply={handleReply} /> </li> ))}</ul>
            </div>
          ) : (
            <>
              <div className="flex justify-center items-center space-x-2 mb-6"> <label className="text-gray-600 text-lg">View Logs for Date:</label> <input type="date" value={adminSelectedDate} onChange={(e) => setAdminSelectedDate(e.target.value)} className="p-3 border bg-gray-50 border-gray-300 rounded-lg text-gray-800 text-lg"/> </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="text-left p-4 border border-gray-200 rounded-xl bg-gray-50"> <h3 className="text-xl font-semibold mb-2 text-gray-900">Daily Requirement Progress</h3> <ul className="space-y-1 text-sm h-40 overflow-y-auto">{Object.entries(adminDailyProgress).map(([name, progress]) => { const qcMet = progress.question_comment >= 2; const rMet = progress.reasoning >= 2; return ( <li key={name} className="flex justify-between items-center pr-2"> <span>{getFirstName(name)}:</span> <span> <span className={qcMet ? 'text-green-500' : 'text-red-500'}>{qcMet ? '✅' : '❌'} {progress.question_comment}/2</span> / <span className={rMet ? 'text-green-500' : 'text-red-500'}>{rMet ? '✅' : '❌'} {progress.reasoning}/2</span> </span> </li> ); })}</ul> </div>
                <div className="text-left p-4 border border-gray-200 rounded-xl bg-gray-50"> <h3 className="text-xl font-semibold text-gray-900">🚦 Daily Understanding Check</h3> <ul className="h-40 overflow-y-auto">{feedbackLog.map((log) => ( <li key={log.id} className="p-2 border-b border-gray-200 text-gray-700">({log.timestamp?.toDate().toLocaleTimeString()}) {getFirstName(log.name)}: {log.status}</li> ))}</ul> </div>
              </div>
              <div className="text-left p-4 border border-gray-200 rounded-xl mt-6 bg-gray-50"> <h3 className="text-xl font-semibold text-gray-900">❓ Daily Posts</h3> <ul className="h-64 overflow-y-auto">{questionsLog.map((log) => ( <li key={log.id} className="p-2 border-b border-gray-200"> <div className="flex justify-between items-start"> <span className="flex-1 mr-2"><span className="font-semibold">{getFirstName(log.name)}</span> [{log.type}]: {log.text}</span> <div className="flex items-center space-x-2 flex-shrink-0"> {log.adminLiked ? <span className="text-green-500 font-bold">✓ Liked</span> : <button onClick={() => handleAdminLike(log.id)} className="text-2xl">👍</button>} <button onClick={() => modifyTalent(log.name, -1, 'penalty')} className="px-2 py-1 bg-red-600 text-white text-xs font-bold rounded hover:bg-red-700">-1</button> <button onClick={() => modifyTalent(log.name, 1, 'bonus')} className="px-2 py-1 bg-yellow-500 text-black text-xs font-bold rounded hover:bg-yellow-600">+1</button> </div> </div> {log.reply && <div className="mt-2 p-2 bg-gray-100 rounded-lg text-sm text-gray-700"><b>Reply:</b> {log.reply} {log.studentLiked && <span className="ml-2">👍 by student</span>}</div>} <ReplyForm log={log} onReply={handleReply} /></li> ))}</ul> </div>
            </>
          )}
          <div className="text-left p-4 border border-gray-200 rounded-xl mt-6 bg-gray-50"> <h3 className="text-xl font-semibold text-gray-900 mb-4">🏆 {selectedCourse} Talent Leaderboard</h3> <TalentGraph talentsData={talentsLog} type="admin" selectedCourse={selectedCourse} getFirstName={getFirstName} /> </div>
        </>
      ) : (
        <>
          <h1 className="text-4xl font-bold text-center mb-2 text-gray-800"><span className="text-green-500">''Ahn''</span>stoppable Learning</h1>
          <p className="text-center text-orange-500 font-semibold text-xl mb-6">Freely Ask, Freely Learn</p>
          <div className="flex flex-wrap justify-center gap-2 my-6"> {COURSES.map((course) => <button key={course} onClick={() => { setSelectedCourse(course); handleNameChange(''); }} className={`px-4 py-2 text-sm font-medium rounded-full ${selectedCourse === course ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>{course}</button>)} </div>
          <select value={nameInput} onChange={(e) => handleNameChange(e.target.value)} disabled={isAuthenticated} className="p-3 mb-2 w-full border bg-gray-50 border-gray-300 rounded-lg text-lg disabled:opacity-50"> <option value="">Select your name...</option> {COURSE_STUDENTS[selectedCourse].map((name, i) => <option key={i} value={name}>{name}</option>)} </select>
          ...
          {isAuthenticated && (
            <div className="mt-4 animate-fade-in">
              <div className="flex justify-center items-center space-x-2 my-4"> <label className="text-gray-600 text-lg">View Logs for Date:</label> <input type="date" value={studentSelectedDate} onChange={(e) => setStudentSelectedDate(e.target.value)} className="p-3 border bg-gray-50 border-gray-300 rounded-lg text-gray-800 text-lg"/> </div>
              {!isClassActive && <div className="text-center p-3 bg-red-100 text-red-800 rounded-lg mb-4"><p>You can only submit new responses during class time.</p></div>}
              <div className={`p-4 border border-gray-200 bg-gray-50 rounded-xl mb-6 ${!isReadyToParticipate ? 'opacity-50 pointer-events-none' : ''}`}>
                <p className="text-xl font-medium text-center text-gray-800">Understanding Check</p>
                <div className="flex justify-center space-x-4 mt-2">
                  <button onClick={() => handleFeedback('Not Understood 🙁')} className={`p-4 w-16 h-16 rounded-full bg-red-500 flex justify-center items-center text-2xl shadow-md hover:shadow-lg transition-shadow ${clickedButton === 'Not Understood 🙁' ? 'ring-4 ring-orange-500' : ''}`}>🙁</button>
                  <button onClick={() => handleFeedback('Confused 🤔')} className={`p-4 w-16 h-16 rounded-full bg-yellow-400 flex justify-center items-center text-2xl shadow-md hover:shadow-lg transition-shadow ${clickedButton === 'Confused 🤔' ? 'ring-4 ring-orange-500' : ''}`}>🤔</button>
                  <button onClick={() => handleFeedback('Got It! ✅')} className={`p-4 w-16 h-16 rounded-full bg-green-500 flex justify-center items-center text-2xl shadow-md hover:shadow-lg transition-shadow ${clickedButton === 'Got It! ✅' ? 'ring-4 ring-orange-500' : ''}`}>✅</button>
                </div>
              </div>
              <div className="text-center p-3 bg-gray-700 text-white rounded-lg mb-4"> <p className="font-bold">Daily Requirement: 4 Talents (2 Q/C + 2 Reasoning)</p> <p className="text-sm">Today's Progress: <span className={`mx-1 ${dailyProgress.question_comment >= 2 ? 'text-green-400' : 'text-red-400'}`}>[{dailyProgress.question_comment}/2 Q/C]</span> <span className={`mx-1 ${dailyProgress.reasoning >= 2 ? 'text-green-400' : 'text-red-400'}`}>[{dailyProgress.reasoning}/2 Reasoning]</span></p> </div>
              <div className={`p-4 border border-gray-200 bg-gray-50 rounded-xl mb-6 ${!isReadyToParticipate ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              <div className="text-left p-4 border border-gray-200 rounded-xl mt-6 bg-gray-50"> <h3 className="text-xl font-semibold text-gray-900 mb-4">Class Score Range</h3> <TalentGraph talentsData={talentsLog} type="student" selectedCourse={selectedCourse} getFirstName={getFirstName} /> </div>
              ...
              <div className="text-left p-4 border border-gray-200 rounded-xl mt-2 bg-gray-50"> <h3 className="text-xl font-semibold text-gray-900 mb-2">My Talent History</h3> <ul className="text-sm space-y-1">{talentTransactions.map((log, i) => ( <li key={i} className={`p-1 flex justify-between items-center ${log.points > 0 ? 'text-green-600' : 'text-red-600'}`}> <span><span className="font-bold">{log.points > 0 ? `+${log.points}` : log.points}</span>: {log.type}</span> <span className="text-xs text-gray-500">({log.timestamp?.toDate().toLocaleDateString()})</span> </li> ))}</ul> </div>
              <div className="text-left p-4 border border-gray-200 rounded-xl mt-6 bg-gray-50"> 
                <h3 className="text-xl font-semibold text-gray-900">Logs for {studentSelectedDate}</h3> 
                <h4 className="font-semibold mt-2 text-gray-700">✍️ My Posts</h4> <ul>{studentActivityLog.map((log) => <li key={log.id} className="p-2 border-b border-gray-200"><div>{log.adminLiked && <span className="mr-2 text-yellow-500 font-bold">👍 by Prof. Ahn</span>}[{log.type}]: {log.text}</div> {log.reply && <div className="mt-2 p-2 bg-gray-100 rounded-lg text-sm text-gray-700 flex justify-between items-center"><span><b>Prof. Ahn's Reply:</b> {log.reply}</span> <button onClick={() => !log.studentLiked && handleStudentLike(log.id)} disabled={log.studentLiked} className="ml-2 text-2xl disabled:opacity-50">{log.studentLiked ? '👍 Liked' : '👍'}</button> </div>} </li>)}</ul>
                <h4 className="font-semibold mt-4 text-gray-700">👀 All Anonymous Posts</h4> <ul className="h-40 overflow-y-auto">{allPostsLog.map((log) => <li key={log.id} className="p-2 border-b border-gray-200"><div>{log.adminLiked && <span className="mr-2">👍</span>}[{log.type}]: {log.text}</div> {log.reply && <div className="mt-2 p-2 bg-gray-100 rounded-lg text-sm text-gray-700"><b>Prof. Ahn's Reply:</b> {log.reply}</div>} </li>)}</ul>
              </div>
            </div>
          )}
        </>
      )}
      <div className="flex flex-col items-center mt-8 pt-4 border-t border-gray-200"> <p className="text-sm font-medium text-gray-500 mb-2">Admin Login</p> <AdminLoginForm onAdminLogin={handleAdminLogin} /> </div>
    </div>
  );
  
  return ( <div className="min-h-screen w-full bg-ivory-background flex items-center justify-center p-4"> <MainContent /> {showMessageBox && ( <div className="fixed top-5 left-1/2 -translate-x-1/2 bg-gray-800 text-white p-4 rounded-xl text-center z-50 shadow-lg"> {message} </div> )} </div> );
};
